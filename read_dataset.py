...
def load_train_data(path):
    ...
    train_df = pd.read_csv(f"{path}/train_FD003.txt", sep=r"\s+", header=None, names=column_names)
...
def load_test_data(path, column_names):
    test_df = pd.read_csv(f"{path}/test_FD003.txt", sep=r"\s+", header=None, names=column_names)
    rul_true = pd.read_csv(f"{path}/RUL_FD003.txt", header=None).values.flatten()
